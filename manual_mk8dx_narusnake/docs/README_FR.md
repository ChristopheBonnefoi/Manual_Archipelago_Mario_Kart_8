...
## État du projet
Le projet est actuellement en **Version 0.12.0 - Mise à jour des checks Time Trial**.
La Version 1.0.0 reste réservée au moment où le projet sera considéré comme terminé.
Cette mise à jour se concentre sur le mode de checks Time Trial, la logique de challenges élargie, le départ de seed, les options fines de contenu, les guides détaillés et une liste d'objectifs de victoire nettoyée, tout en conservant la gestion des variantes de personnages, les options DLC, la configuration MKTV Token et la base stable Manual Archipelago déjà rafraîchie (`manual_stable_20260319`).
...
- **Options de cylindrée**
  Activer ou désactiver les checks 50cc, 100cc, 150cc, Mirror et 200cc depuis le YAML. `difficulty_items` peut aussi retirer totalement la progression par cylindrée et remplacer les checks avec cylindrée par des checks propres sans difficulté.
...
- **Guides détaillés**
  Des guides explicatifs sont disponibles dans `manual_mk8dx_narusnake/docs/guide_en/` et `manual_mk8dx_narusnake/docs/guide_fr/`.
...
- Ajout de `difficulty_items` pour retirer la progression par cylindrée, cacher les checks avec cylindrée et utiliser des variantes propres sans difficulté.
